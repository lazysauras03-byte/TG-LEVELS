...
//   // For BEARISH crossover: look for BULLISH BCVC, then immediate BEARISH BCVC that crosses its low
//   else if (latestCrossover.type === "BEARISH_CROSSOVER") {
//     // Find first BULLISH BCVC after crossover
//     const firstBullishIndex = formationsAfterCrossover.findIndex(
//       (f) => f.isBullish,
//     );
...
//       return {
//         found: false,
//         reason: "No BULLISH BCVC found after the bearish crossover",
//       };
//     }

//     const firstBullishBCVC = formationsAfterCrossover[firstBullishIndex];

//     // Check if there's an immediate next BCVC (no gap)
//     if (firstBullishIndex + 1 >= formationsAfterCrossover.length) {
//       return {
//         found: false,
//         reason: "No BCVC found after the BULLISH BCVC",
//       };
//     }

//     const nextBCVC = formationsAfterCrossover[firstBullishIndex + 1];

//     // Must be BEARISH and immediately after (no other BCVC in between)
//     if (!nextBCVC.isBearish) {
//       return {
//         found: false,
//         reason: "Next BCVC after BULLISH is not BEARISH",
//       };
//     }

//     // Check if BEARISH BCVC crosses the BULLISH BCVC's low
//     const bullishLow = firstBullishBCVC.low;
//     const bearishCrossedLow =
//       nextBCVC.low < bullishLow || nextBCVC.close < bullishLow;

//     if (!bearishCrossedLow) {
//       return {
//         found: false,
//         reason: `BEARISH BCVC (low: ${nextBCVC.low}, close: ${nextBCVC.close}) did not cross BULLISH BCVC low (${bullishLow})`,
//       };
//     }
...
//       crossover: latestCrossover,
//       bullishBCVC: firstBullishBCVC,
//       bearishBCVC: nextBCVC,
//       validation: {
//         isImmediate: true,
//         bullishLow: bullishLow,
//         bearishLow: nextBCVC.low,
//         bearishClose: nextBCVC.close,
//         crossedLow: bearishCrossedLow,
//       },
//       summary: {
//         crossoverTime: latestCrossover.timestamp,
//         bullishTime: firstBullishBCVC.timestamp,
//         bearishTime: nextBCVC.timestamp,
//         crossoverPrice: latestCrossover.price,
//         bullishClose: firstBullishBCVC.close,
//         bullishLow: bullishLow,
//         bearishClose: nextBCVC.close,
//         bearishLow: nextBCVC.low,
//       },
...
//     console.log(symbols)
//     // Get current time and 5 days ago using moment
//     const now = moment();
...
//     const BATCH_SIZE = 20; // Process 20 stocks before waiting
//     const WAIT_TIME = 15000; // Wait 60 seconds (1 minute) between batches

//     console.log(`\n🕐 Current Time: ${now.format('YYYY-MM-DD HH:mm:ss')}`);
//     console.log(`📅 Checking for crossovers since: ${fiveDaysAgo.format('YYYY-MM-DD HH:mm:ss')}`);
//     console.log(`⚙️  Rate Limiting: Processing ${BATCH_SIZE} symbols, then waiting ${WAIT_TIME / 1000} seconds`);
...
//         // Check if crossover is within last 5 days
//         if (crossoverTime.isBefore(fiveDaysAgo)) {
//           const daysAgo = now.diff(crossoverTime, 'days');
//           const hoursAgo = now.diff(crossoverTime, 'hours');
//           console.log(`⏭️  Skipping ${symbol}: Latest crossover is ${daysAgo} days (${hoursAgo} hours) old`);
//           console.log(`   Crossover: ${latestCrossover.type} at ${crossoverTime.format('YYYY-MM-DD HH:mm')}`);
//           continue;
//         }
...
//         console.log(`  Time: ${crossoverTime.format('YYYY-MM-DD HH:mm:ss')}`);
//         console.log(`  Age: ${daysAgo} days, ${hoursAgo % 24} hours, ${minutesAgo % 60} minutes ago`);
//         console.log(`  Relative: ${crossoverTime.fromNow()}`);

//         // Now fetch BCVC data since we have a recent crossover
//         console.log(`📊 Fetching BCVC data for ${symbol}...`);
//         const bcvc = await bcvcManager.getHistoricalBCVC(symbol, "60", 50);
//         // console.log(bcvc)
...
//             );
//             console.log(
//               `🚀 Bullish BCVC: ${pattern.bullishBCVC.timestamp} (Low: ${pattern.bullishBCVC.low})`,
//             );
//             console.log(
//               `🔴 Bearish BCVC: ${pattern.bearishBCVC.timestamp} (Low: ${pattern.bearishBCVC.low}) - CROSSED BULLISH LOW ✓`,
//             );
//           }
//         } else {
...
//     console.log(`Recent crossovers (within 5 days): ${recentCrossovers}`);
//     console.log(`Patterns found: ${patternsFound}`);
//     console.log(`Success rate: ${recentCrossovers > 0 ? ((patternsFound / recentCrossovers) * 100).toFixed(2) : 0}%`);
//     console.log('='.repeat(60));
...
const startlogic = async () => {
  try {
    ...
    // Rate limiting configuration
    const BATCH_SIZE = 20; // Process 20 stocks before waiting
    const WAIT_TIME = 15000; // Wait 15 seconds between batches

    console.log(`\n🕐 Current Time: ${now.format("YYYY-MM-DD HH:mm:ss")}`);
    console.log(
      `📅 Checking for crossovers since: ${fiveDaysAgo.format("YYYY-MM-DD HH:mm:ss")}`,
    );
    console.log(
    ...
    for (let i = 0; i < Math.min(208, symbols.length); i++) {
      ...
      try {
        ...
        if (crossoverTime.isBefore(fiveDaysAgo)) {
          ...
          const hoursAgo = now.diff(crossoverTime, "hours");
          console.log(
            `⏭️  Skipping ${symbol}: Latest crossover is ${daysAgo} days (${hoursAgo} hours) old`,
          );
          console.log(
            `   Crossover: ${latestCrossover.type} at ${crossoverTime.format("YYYY-MM-DD HH:mm")}`,
          );
          continue;
        }
        ...
        console.log(`  Time: ${crossoverTime.format("YYYY-MM-DD HH:mm:ss")}`);
        console.log(
          `  Age: ${daysAgo} days, ${hoursAgo % 24} hours, ${minutesAgo % 60} minutes ago`,
        );
        console.log(`  Relative: ${crossoverTime.fromNow()}`);
        ...
        if (latestCrossover.type === "BEARISH_CROSSOVER") {
          // For bearish crossover, include red candles
          console.log(
            `  🔻 Bearish crossover detected - including RED candles`,
          );
          bcvc = await bcvcManager.getHistoricalBCVC(symbol, "60", 50, "red");
        } else {
          // For bullish crossover, normal behavior (white & orange only)
          console.log(
            `  🚀 Bullish crossover detected - normal BCVC (white & orange)`,
          );
          bcvc = await bcvcManager.getHistoricalBCVC(symbol, "60", 50);
        }
