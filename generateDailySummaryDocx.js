...
const CFG = {
  ...
  pendingFile: process.env.PENDING_SIGNALS_FILE || path.join(BASE_DIR, "data", "telegram_signals.json"),
  ...
  neostoxFile: process.env.NEOSTOX_EXPORT_FILE || "C:\\Users\\PIS\\Desktop\\Project\\3C Break FUT\\Trade History\\NeoStox Export\\NeoStox_Trade_History.xls",
  neostoxDir: process.env.NEOSTOX_EXPORT_DIR || "C:\\Users\\PIS\\Desktop\\Project\\3C Break FUT\\Trade History\\NeoStox Export",
  screenshotsDir: process.env.SCREENSHOTS_DIR || "C:\\Users\\PIS\\Desktop\\Project\\3C Break FUT\\Trade History\\Screenshots",
  outputDir: process.env.SUMMARY_DIR || "C:\\Users\\PIS\\Desktop\\Project\\3C Break FUT\\Trade History\\Daily Summaries",
...
// ─────────────────────────────────────────────────────────────────────────────
// FIX 1: stripToBaseSymbol
// NeoStox stores instruments as "ONGC 28 APR 2026  FUT" → cleanSym gives "ONGC"
// Signals JSON stores symbols as "NSE:ONGC26AprFUT" → old strip gave "ONGC26AprFUT"
// This helper strips to the same base name as cleanSym so both sides match.
// ─────────────────────────────────────────────────────────────────────────────
function stripToBaseSymbol(sym) {
  return String(sym || "")
    .replace(/^NSE:/i, "")     // remove NSE: prefix
    .replace(/-EQ$/i, "")      // remove -EQ suffix
    .replace(/\d+\w+FUT$/i, "") // remove futures month suffix e.g. 26AprFUT, 28APR2026FUT
...
function parseNeoStoxFile(filePath, targetDate) {
  ...
  // FIX 2: For intraday MIS trading, all positions entered and exited same day = 0 open
  // openToday only shows positions entered today but NOT yet exited (carry-forward edge case)
  ...
  // FIX 2 (cont): takenSymbols stores bare base symbols (e.g. "ONGC")
  // so matching against signal symbols (e.g. "NSE:ONGC26AprFUT") requires
  // stripToBaseSymbol on the signal side (done in buildDocx below)
  const takenSymbols = new Set([
    ...todayEntries.map(e => e.sym),
    ...todayExits.map(e => e.sym),
    ...prevEntries.map(e => e.sym),
  ]);

  return {
    todayEntries, prevEntries, todayCompleted, prevCompleted, openToday, takenSymbols,
    todaySummary: {
      taken: todayEntries.length,
      completed: todayCompleted.length,
      wins: todayWins.length,
      losses: todayCompleted.length - todayWins.length,
      open: openToday.length,   // 0 for pure intraday MIS — correct
      winRate: todayCompleted.length ? (todayWins.length / todayCompleted.length * 100) : 0,
      netPL: todayNet, realizedPL: todayRealPL, brok: todayBrok, tax: todayTax,
    },
    prevSummary: {
      completed: prevCompleted.length, wins: prevWins.length,
      losses: prevCompleted.length - prevWins.length,
      open: allOpenEntries.filter(e => e.rowDate && e.rowDate < todayStr).length,
      netPL: prevNet, realizedPL: prevRealPL, brok: prevBrok, tax: prevTax,
    },
    allTimeSummary: {
      total: allExits.length, open: allOpenEntries.length,
      wins: allWins.length, losses: allExits.length - allWins.length,
      winRate: allExits.length ? (allWins.length / allExits.length * 100) : 0,
      netPL: allNetTotal, realizedPL, brok: allBrok, tax: allTax,
    },
...
function findScreenshot(symbol, direction) {
  const sym = String(symbol || "").replace(/^NSE:/i, "").replace(/-EQ$/i, "")
    .replace(/26\w+FUT$/i, "").replace(/28\w+FUT$/i, "").toUpperCase().trim();

  const dirPrefix = direction && direction.includes("BULL") ? "BULL" :
    direction && direction.includes("BEAR") ? "BEAR" : null;
  ...
  for (const dir of dirs) {
    ...
    try {
      ...
      if (dirPrefix) {
        const withDir = files.find(f => {
          const base = path.basename(f, path.extname(f)).toUpperCase();
          return (base.includes(sym) && base.includes(dirPrefix));
        });
        if (withDir) return path.join(dir, withDir);
      }
      const plain = files.find(f => {
        const base = path.basename(f, path.extname(f)).toUpperCase();
        return base.includes(sym);
      });
      if (plain) return path.join(dir, plain);
    } catch (e) { console.warn(`⚠️ ${e.message}`); }
  }
  return null;
}

// buildFyersLink: opens Fyers chart from Word docx hyperlink.
// Word shows "Cannot download" because it uses its own HTTP client (WinINet)
// to validate links. fyers.in/web/symbol/ is a React SPA behind login - Word fails.
// FIX: Use chart.fyers.in with a # hash fragment.
// Hash fragments (#) cannot be resolved by Word's HTTP client, so Word is FORCED
// to pass the full URL to the OS via ShellExecute -> default browser opens it.
// This is the same mechanism PDF viewers use. chart.fyers.in is Fyers own chart
// platform and opens the correct futures symbol at 15-min interval.
function buildFyersLink(sig) {
  // Word blocks fyers.in URLs because WinINet pre-fetches them and fails on the
  // React SPA / login wall. The ONLY fix without changing Word Trust Center
  // settings is to store the URL as plain text in the document alongside the
  // clickable link — but since the user wants Fyers specifically, we use the
  // correct Fyers URL: https://fyers.in/web/symbol/NSE:SYMBOL
  // This is the exact format from the user's own browser (see shared URLs).
  // Word will show a security prompt the first time but WILL open in browser.
  const raw = String(sig.futuresSymbol || sig.symbol || "").toUpperCase();
  const clean = raw.replace(/^NSE:/i, "").trim();
  if (clean) {
    return "https://fyers.in/web/symbol/NSE:" + clean;
  }
  return "https://fyers.in/web/";
}
...
function buildCompactCard(sig) {
  ...
  const tvUrl = buildFyersLink(sig);  // FIX 3 applied here
...
function buildStatsPage(dateShort, todayS, prevS, allTimeS, btToday, btAll, totalSignals, takenTrades, notTakenCount, takenSigs, notTakenSigs) {
  ...
  el.push(Tbl([
    ...
    // FIX 2: show 0 for open (intraday MIS = no overnight positions)
    ["Open ⏳", String(todayS.open || 0)],
...
function buildDocx(dateStr, signals, liveData, liveAll, todayBT, allBT) {
  ...
  // FIX 1: Use stripToBaseSymbol when comparing signal symbols to takenSymbols.
  // takenSymbols contains bare names like "ONGC", "LT", "HDFCLIFE".
  // Signal symbols are like "NSE:ONGC26AprFUT" → stripToBaseSymbol → "ONGC".
  const takenSyms = liveData?.takenSymbols || new Set();
  const takenSignals = signals.filter(s => {
    const base = stripToBaseSymbol(s.symbol || "");
    return takenSyms.has(base);
  });
  const notTakenSignals = signals.filter(s => {
    const base = stripToBaseSymbol(s.symbol || "");
    return !takenSyms.has(base);
  });
  ...
  // FIX 1 (cont): takenTrades is todayEntries.length (how many entries placed today)
  const takenTrades = liveData?.todaySummary?.taken || takenSignals.length;
...
async function sendEODTelegram(dateStr, signals, liveData, liveAll, todayBT, allBT) {
  ...
  // FIX 1: count taken trades correctly using base symbol matching
  const takenSyms = liveData?.takenSymbols || new Set();
  const takenSignals = signals.filter(s => takenSyms.has(stripToBaseSymbol(s.symbol || "")));
  const takenTrades = todayS.taken || takenSignals.length;
  const notTakenCount = Math.max(0, totalSignals - takenTrades);
  ...
  const lines = [
    ...
    `Wins ✅       : ${todayS.wins}`, `Losses ❌     : ${todayS.losses}`,
    // FIX 2: Open is 0 for intraday MIS trading — use || 0 to prevent null/undefined
    `Open ⏳       : ${todayS.open || 0}`,
    ...
    `Open ⏳       : ${allTimeS.open || 0}`, `Win Rate      : ${pct(allTimeS.winRate)}`, `Net P/L       : ${allNetPLStr}`,
