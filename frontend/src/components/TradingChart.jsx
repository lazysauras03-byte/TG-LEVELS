...
//   // Last candle: solid filled bubbles (same shape as NH/NL), orange color
//   LAST_HIGH: { color: "#f97316", shape: "circle", position: "aboveBar", size: 1.5 },
//   LAST_LOW: { color: "#f97316", shape: "circle", position: "belowBar", size: 1.5 },
// };
...
//   // Multi-day (3M) mode: only NH/NL shown — FH/FL/LH/LL hidden; NH/NL continuous (not reset per day)
...
//   // Rolling counter 1→20 for all timeframes, reset after 20
//   let counter = 0;
//   return markerList.map(m => {
//     counter++;
//     const num = ((counter - 1) % 20) + 1;
//     return {
//       time: m.time,
//       position: m.position,
//       color: m.color,
...
export default function TradingChart({ data, showEMA9High, showEMA9Low, showEMA9Close, onCrosshairMove, todayMode }) {
  ...
  useEffect(() => { seriesRef.current.ema9HighSeries?.applyOptions({ visible: showEMA9High ?? true }); }, [showEMA9High]);
  useEffect(() => { seriesRef.current.ema9LowSeries?.applyOptions({ visible: showEMA9Low ?? true }); }, [showEMA9Low]);
  useEffect(() => { seriesRef.current.ema9CloseSeries?.applyOptions({ visible: showEMA9Close ?? false }); }, [showEMA9Close]);
...
function buildMarkers(signals, sortedCandleTimes, todayMode) {
  if (!signals || signals.length === 0) return [];

  const sorted = [...signals].sort((a, b) => a.ts - b.ts);

  // In multi-day (1-month) mode: show ALL signal types but NH/NL only across days
  // In today mode: show all types (FH/FL/NH/NL/LH/LL) for today only
  const uniqueDays = new Set(sorted.map(s => tsToISTDate(s.ts)));
  const isMultiDay = uniqueDays.size > 1;

  // Multi-day: only NH/NL — FH/FL/LH/LL removed
  ...
  for (const sig of sorted) {
    const style = SIGNAL_STYLES[sig.type];
    if (!style) continue;
    if (isMultiDay && !MULTIDAY_VISIBLE.has(sig.type)) continue;
  ...
  // Rolling 1→20 for ALL timeframes, resets after 20
  let counter = 0;
  return markerList.map(m => {
    counter++;
    const num = ((counter - 1) % 20) + 1;
